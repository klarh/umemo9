(() => {
  ...
  window._showToast = function(msg, duration) {
  ...
  window._hideToast = function() {
    var el = document.getElementById("status-toast");
    if (el) el.classList.remove("status-toast--visible");
    if (_toastTimer) {
  ...
  var registerDeleteDeckHandler = function(model) {
    return function() {
      window._deleteDeck = function(el) {
        ...
        if (!Port) return;
        if (window._markDeckLocallyDeleted) {
          window._markDeckLocallyDeleted(deckId);
        }
  ...
  function voteFromJs(model, ps, delta) {
    ...
    Port.getCardFaces(deckId)(cardId)(function(facesJson) {
      return function() {
        ...
        Port.getActiveFaceCount(deckId)(function(dActive) {
          return function() {
            var studyIntensity = model.settings.learningRate;
            var R = Math.max(1, studyIntensity - 1);
            var c = dActive > 1 ? Math.pow(dActive, 1 / R) : Math.max(2, studyIntensity);
            c = Math.max(1.5, c);
            ...
            var frontFaceId = ps.frontFace && ps.frontFace.value0 ? ps.frontFace.value0.faceId : null;
            var updatedFaces = facesJson.map(function(f) {
              var copy = Object.assign({}, f);
              var fLinked = f.contents && f.contents.link_siblings != null ? f.contents.link_siblings : true;
              var shouldUpdate = f.faceId === frontFaceId || linked && fLinked;
              if (shouldUpdate) {
                copy.momentum = newMomentum;
                if (copy.contents) {
                  copy.contents = Object.assign({}, copy.contents);
                  copy.contents.momentum = newMomentum;
                }
              }
              return copy;
            });
            Port.getPositionsAroundRank(deckId)(rank)(function(posInfo) {
              return function() {
                ...
                var finalFaces = updatedFaces.map(function(f) {
                  var copy = Object.assign({}, f);
                  var fLinked = f.contents && f.contents.link_siblings != null ? f.contents.link_siblings : true;
                  var shouldMove = f.faceId === frontFaceId || linked && fLinked;
                  if (shouldMove) {
                    var noise = 1 + (Math.random() - 0.5) * 1e-5;
                    copy.position = newPosition * noise;
                  }
                  copy._ts = now;
                  return copy;
                });
                Port.saveFacesWithCallback(finalFaces)(function() {
                  return function() {
                    if (window._appendJournal) {
                      for (var ji = 0; ji < finalFaces.length; ji++) {
                        var jf = finalFaces[ji];
                        var jfLinked = jf.contents && jf.contents.link_siblings != null ? jf.contents.link_siblings : true;
                        var moved = jf.faceId === frontFaceId || linked && jfLinked;
                        if (moved) {
  ...
  function _showNextCard(model, ps) {
    ...
    Port.getFrontCardSpeculative(ps.deckId)(function(result) {
      return function() {
        ...
        var frontType = (frontFace.contents || {}).type || "TextFace";
        var isDrawFront = frontType === "DrawFace";
        var html = "";
        var promptFaceId = null;
        if (isDrawFront) {
          var nonDraw = result.faces.filter(function(f2) {
            return f2.faceId !== result.frontFaceId && ((f2.contents || {}).type || "TextFace") !== "DrawFace";
          });
          html += '<div class="face play-face" id="answer-face">';
          if (nonDraw.length > 0) {
            var priorities = nonDraw.map(function(f2) {
              return f2.contents && f2.contents.priority || 1;
            });
            var sum2 = 0;
            for (var pi = 0; pi < priorities.length; pi++) sum2 += priorities[pi];
            var q = Math.random(), cumSum = 0, idx = 0;
            for (var pj = 0; pj < priorities.length; pj++) {
              cumSum += priorities[pj];
              if (cumSum / sum2 > q) {
                idx = pj;
                break;
              }
            }
            var prompt = nonDraw[idx];
            promptFaceId = prompt.faceId;
            html += _renderFaceContent(prompt.contents || {});
          }
          html += _renderFaceContent(frontFace.contents || {});
          html += "</div>";
        }
        for (var i = 0; i < result.faces.length; i++) {
          var f = result.faces[i];
          var c = f.contents || {};
          var fType = c.type || "TextFace";
          if (isDrawFront) {
            if (f.faceId === result.frontFaceId || f.faceId === promptFaceId || fType === "DrawFace") continue;
          } else {
            if (fType === "DrawFace" && f.faceId !== result.frontFaceId) continue;
          }
          var isFront = !isDrawFront && f.faceId === result.frontFaceId;
  ...
  function _initDrawCanvas() {
    ...
    _drawCanvas = canvas;
    _sizeCanvas(canvas);
    _drawOrigW = canvas.clientWidth;
    _drawOrigH = canvas.clientHeight;
    var ctx = canvas.getContext("2d");
    _drawCtx = ctx;
    function getPos(e) {
      var r = canvas.getBoundingClientRect();
      var bdr = (r.width - canvas.clientWidth) / 2;
      var t = e.touches ? e.touches[0] : e;
      return { x: (t.clientX - r.left - bdr) / canvas.clientWidth, y: (t.clientY - r.top - bdr) / canvas.clientHeight };
    }
    function onStart(e) {
      if (_drawFrozen) return;
      e.preventDefault();
      _drawCurrent = [getPos(e)];
    }
    function onMove(e) {
      if (_drawFrozen || !_drawCurrent) return;
      e.preventDefault();
      var p = getPos(e);
      _drawCurrent.push(p);
      var cw = canvas.clientWidth, ch = canvas.clientHeight;
      var prev = _drawCurrent[_drawCurrent.length - 2];
      ctx.lineCap = "round";
      ctx.lineJoin = "round";
      ctx.lineWidth = 2.5;
      ctx.strokeStyle = "#000";
    ...
    function onEnd(e) {
      if (_drawFrozen || !_drawCurrent) return;
      if (_drawCurrent.length > 0) {
        _drawStrokes.push(_drawCurrent);
      }
      _drawCurrent = null;
    }
    ...
    window._drawUndo = function() {
      if (_drawFrozen || _drawStrokes.length === 0) return;
      _drawStrokes.pop();
      _redrawCanvas();
    };
    window._drawClear = function() {
      if (_drawFrozen) return;
      _drawStrokes = [];
      _redrawCanvas();
    };
  ...
  function _redrawCanvas() {
    ...
    ctx.lineWidth = 2.5;
    ctx.strokeStyle = "#000";
    for (var s = 0; s < _drawStrokes.length; s++) {
      var stroke = _drawStrokes[s];
      if (stroke.length < 2) continue;
      ctx.beginPath();
      ctx.moveTo(stroke[0].x * w, stroke[0].y * h);
      for (var p = 1; p < stroke.length; p++) {
        ctx.lineTo(stroke[p].x * w, stroke[p].y * h);
      }
      ctx.stroke();
    }
  }
  var initDrawCanvas = _initDrawCanvas;
  var freezeDrawCanvas = function() {
    _drawFrozen = true;
    var canvas = document.getElementById("draw-canvas");
    if (!canvas) return;
    var tools = canvas.parentElement && canvas.parentElement.querySelector(".draw-tools");
    if (tools) tools.style.display = "none";
    canvas.style.cursor = "default";
    if (_drawStrokes.length === 0) {
      canvas.style.display = "none";
      return;
    }
    var minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
    for (var s = 0; s < _drawStrokes.length; s++) {
      for (var p = 0; p < _drawStrokes[s].length; p++) {
        var px = _drawStrokes[s][p].x * _drawOrigW;
        var py = _drawStrokes[s][p].y * _drawOrigH;
        if (px < minX) minX = px;
        if (py < minY) minY = py;
        if (px > maxX) maxX = px;
        if (py > maxY) maxY = py;
    ...
    var bw = maxX - minX || 1;
    var bh = maxY - minY || 1;
    var pad = Math.max(bw, bh) * 0.12;
    minX -= pad;
    minY -= pad;
    bw += pad * 2;
    bh += pad * 2;
    var containerW = canvas.parentElement ? canvas.parentElement.getBoundingClientRect().width : _drawOrigW;
    var borderW = 3;
    var usableW = containerW - borderW;
    var targetH = usableW * (bh / bw);
    targetH = Math.min(targetH, 300);
    ...
    setTimeout(function() {
      ...
      var offX = (cw - bw * scale) / 2;
      var offY = (ch - bh * scale) / 2;
      ctx.lineCap = "round";
      ctx.lineJoin = "round";
      ctx.lineWidth = 2.5;
      ctx.strokeStyle = "#000";
      for (var s2 = 0; s2 < _drawStrokes.length; s2++) {
        var stroke = _drawStrokes[s2];
        if (stroke.length < 2) continue;
        ctx.beginPath();
        ctx.moveTo(
          offX + (stroke[0].x * _drawOrigW - minX) * scale,
          offY + (stroke[0].y * _drawOrigH - minY) * scale
        );
        for (var p2 = 1; p2 < stroke.length; p2++) {
          ctx.lineTo(
            offX + (stroke[p2].x * _drawOrigW - minX) * scale,
            offY + (stroke[p2].y * _drawOrigH - minY) * scale
          );
        }
        ctx.stroke();
      }
  ...
  function _idbFaceToFace(rec) {
    ...
    return {
      ...
      alt: c.alt || "",
      label: c.label || "",
      isDrawFace: (c.faceType || c.type) === "DrawFace"
    };
  }
  function _renderFaceContent(c) {
    ...
    if (type === "DrawFace") {
      var label = c.label ? '<p class="face-text draw-label">' + _escapeHtml(c.label) + "</p>" : "";
      return label + '<canvas class="draw-canvas" id="draw-canvas"></canvas><div class="draw-tools"><button onclick="window._drawUndo()">Undo</button><button onclick="window._drawClear()">Clear</button></div>';
    }
  ...
  var registerEditHandlers = function(deckId) {
    return function() {
      ...
      _registerBulkActions(deckId);
    };
  };
  var _bulkMatched = [];
  function _registerBulkActions(deckId) {
    var filterEl = document.getElementById("bulk-filter");
    var extraEl = document.getElementById("bulk-filter-extra");
    var actionEl = document.getElementById("bulk-action");
    var actionExtraEl = document.getElementById("bulk-action-extra");
    if (!filterEl) return;
    window._compactDeck = function() {
      _compactDeckFaces(deckId);
    };
    window._rebalanceDeck = function() {
      var Port = window._portModule;
      if (!Port) return;
      var el = document.getElementById("compact-result");
      if (el) el.textContent = "Rebalancing\u2026";
      Port.rebalanceDeck(deckId)(function() {
        if (el) el.textContent = "\u2713 Positions rebalanced.";
      })();
    };
    window._groupByCard = function() {
      var Port = window._portModule;
      if (!Port) return;
      var el = document.getElementById("compact-result");
      if (el) el.textContent = "Grouping\u2026";
      Port.getAllDeckFaces(deckId)(function(faces) {
        return function() {
          var cardMin = {};
          for (var i = 0; i < faces.length; i++) {
            var f = faces[i];
            if (f.position != null) {
              if (cardMin[f.cardId] === void 0 || f.position < cardMin[f.cardId]) {
                cardMin[f.cardId] = f.position;
              }
            }
          }
          var updated = [];
          for (var j = 0; j < faces.length; j++) {
            var face = faces[j];
            var minPos = cardMin[face.cardId];
            if (minPos !== void 0 && (face.position == null || face.position !== minPos)) {
              var copy = Object.assign({}, face);
              copy.position = minPos;
              updated.push(copy);
            }
          }
          if (updated.length === 0) {
            if (el) el.textContent = "All faces already grouped.";
            return;
          }
          Port.saveFacesWithCallback(updated)(function() {
            return function() {
              if (el) el.textContent = "Grouped " + updated.length + " faces. Rebalancing\u2026";
              Port.rebalanceDeck(deckId)(function() {
                if (el) el.textContent = "\u2713 Grouped " + updated.length + " faces and rebalanced.";
              })();
            };
          })();
        };
    ...
    filterEl.addEventListener("change", function() {
      ...
      if (v === "type") {
        extraEl.innerHTML = ' <select id="bulk-filter-type"><option value="TextFace">Text</option><option value="MarkdownFace">Markdown</option><option value="ImageFace">Image</option><option value="AudioFace">Audio</option><option value="DrawFace">Draw</option></select>';
      } else if (v === "momentum-range") {
        extraEl.innerHTML = ' <input id="bulk-filter-mom-min" type="number" step="0.1" value="0" style="width:4em"> \u2013 <input id="bulk-filter-mom-max" type="number" step="0.1" value="1" style="width:4em">';
      } else if (v === "text-contains") {
        extraEl.innerHTML = ' <input id="bulk-filter-text" type="text" placeholder="search text" style="width:10em">';
      } else {
        extraEl.innerHTML = "";
      }
      _bulkMatched = [];
    ...
    actionEl.addEventListener("change", function() {
      ...
      if (v === "set-momentum") {
        actionExtraEl.innerHTML = ' <input id="bulk-action-min" type="number" step="0.1" value="0.5" style="width:4em"> \u2013 <input id="bulk-action-max" type="number" step="0.1" value="1.5" style="width:4em">';
      } else if (v === "set-priority") {
        actionExtraEl.innerHTML = ' <input id="bulk-action-min" type="number" step="0.1" value="0.5" style="width:4em"> \u2013 <input id="bulk-action-max" type="number" step="0.1" value="1.5" style="width:4em">';
      } else {
        actionExtraEl.innerHTML = "";
      }
    });
    window._bulkPreview = function() {
      var Port = window._portModule;
      if (!Port) return;
      ...
      Port.getAllDeckFaces(deckId)(function(faces) {
        return function() {
          var matched = _bulkFilterFaces(faces);
          _bulkMatched = matched;
          _setBulkCount(matched.length);
          if (resultEl) resultEl.textContent = matched.length + " of " + faces.length + " faces matched.";
        };
      })();
    };
    window._bulkApply = function() {
      if (_bulkMatched.length === 0) return;
      var action = actionEl.value;
      var updated = [];
      var PRIORITY_MAX = 2147483647;
      for (var i = 0; i < _bulkMatched.length; i++) {
        var f = Object.assign({}, _bulkMatched[i]);
        f.contents = Object.assign({}, f.contents || {});
        switch (action) {
          case "link":
            f.contents.link_siblings = true;
            break;
          case "unlink":
            f.contents.link_siblings = false;
            break;
          case "set-momentum":
            var momMin = parseFloat((document.getElementById("bulk-action-min") || {}).value);
            var momMax = parseFloat((document.getElementById("bulk-action-max") || {}).value);
            if (isNaN(momMin) || isNaN(momMax)) {
              alert("Invalid momentum range");
              return;
            }
            var momRange = momMax - momMin;
            var momVal = _bulkMatched.length === 1 ? (momMin + momMax) / 2 : momMin + momRange * (i / (_bulkMatched.length - 1));
            f.momentum = momVal;
            break;
          case "set-priority":
            var priMin = parseFloat((document.getElementById("bulk-action-min") || {}).value);
            var priMax = parseFloat((document.getElementById("bulk-action-max") || {}).value);
            if (isNaN(priMin) || isNaN(priMax)) {
              alert("Invalid priority range");
              return;
            }
            var priRange = priMax - priMin;
            var priVal = _bulkMatched.length === 1 ? (priMin + priMax) / 2 : priMin + priRange * (i / (_bulkMatched.length - 1));
            f.contents.priority = priVal;
            break;
          case "reset-position":
            break;
        }
      ...
      if (action === "reset-position") {
        ...
        for (var j = 0; j < updated.length; j++) {
          if (updated[j].position != null) cardIds[updated[j].cardId] = true;
        }
        var cardList = Object.keys(cardIds);
        for (var k = cardList.length - 1; k > 0; k--) {
          var r = Math.floor(Math.random() * (k + 1));
          var tmp = cardList[k];
          cardList[k] = cardList[r];
          cardList[r] = tmp;
        }
        var step = PRIORITY_MAX / (cardList.length + 1);
        var posMap = {};
        for (var m = 0; m < cardList.length; m++) {
          posMap[cardList[m]] = step * (m + 1);
        }
        for (var n = 0; n < updated.length; n++) {
          if (updated[n].position != null && posMap[updated[n].cardId] != null) {
            updated[n].position = posMap[updated[n].cardId];
          }
        }
      }
      if (!confirm('Apply "' + action + '" to ' + updated.length + " faces?")) return;
      var Port = window._portModule;
      Port.saveFaces(updated)();
      var ds = window._driveSyncModule;
      var contentActions = { link: 1, unlink: 1, "set-priority": 1 };
      if (contentActions[action] && ds && ds.markContentDirty) {
        ds.markContentDirty(deckId);
      }
      var appendJ = window._appendJournal;
      if (appendJ && (action === "set-momentum" || action === "reset-position")) {
        for (var p = 0; p < updated.length; p++) {
          var uf = updated[p];
          if (uf.position != null && uf._ts) {
            appendJ(deckId, {
              faceId: uf.faceId,
              cardId: uf.cardId,
              momentum: uf.momentum || 1,
      ...
      var resultEl = document.getElementById("bulk-result");
      if (resultEl) resultEl.textContent = "\u2713 Updated " + updated.length + " faces. Reload page to see changes.";
      _bulkMatched = [];
      _setBulkCount(0);
    };
  }
  function _bulkFilterFaces(faces) {
    var filterEl = document.getElementById("bulk-filter");
    var filter3 = filterEl ? filterEl.value : "all";
    return faces.filter(function(f) {
      var c = f.contents || {};
      var linked = c.link_siblings != null ? c.link_siblings : true;
      var fType = c.type || "TextFace";
      var mom = f.momentum != null ? f.momentum : c.momentum != null ? c.momentum : 1;
      switch (filter3) {
        case "linked":
          return linked;
        case "unlinked":
          return !linked;
        case "type":
          var sel = document.getElementById("bulk-filter-type");
          return sel && fType === sel.value;
        case "momentum-range":
          var minEl = document.getElementById("bulk-filter-mom-min");
          var maxEl = document.getElementById("bulk-filter-mom-max");
          var lo = minEl ? parseFloat(minEl.value) : 0;
          var hi = maxEl ? parseFloat(maxEl.value) : Infinity;
          return mom >= lo && mom <= hi;
        case "text-contains":
          var q = (document.getElementById("bulk-filter-text") || {}).value || "";
          if (!q) return true;
          var haystack = (c.text || "") + " " + (c.label || "") + " " + (c.src || "");
  ...
  function _setBulkCount(n) {
    var countEl = document.getElementById("bulk-apply-count");
    var btn = document.getElementById("bulk-apply-btn");
    if (countEl) countEl.textContent = n;
    if (btn) btn.disabled = n === 0;
    var matchEl = document.getElementById("bulk-match-count");
    if (matchEl) matchEl.textContent = n > 0 ? n + " faces selected" : "";
  }
  function _compactDeckFaces(deckId) {
    var statusEl = document.getElementById("compact-result");
    function setStatus(msg) {
      if (statusEl) statusEl.textContent = msg;
    }
    if (!confirm("Compact will reassign all face IDs to be contiguous and force a full re-sync on next push. Continue?")) return;
    var Port = window._portModule;
    if (!Port) {
      setStatus("Error: port module not loaded");
      return;
    }
    setStatus("Reading faces\u2026");
    Port.getDeckFaces(deckId)(function(positionedFaces) {
      return function() {
        var dbName = "umemo-deck-" + deckId;
        var openReq = indexedDB.open(dbName);
        openReq.onsuccess = function() {
          var db = openReq.result;
          var tx = db.transaction("faces", "readonly");
          var allReq = tx.objectStore("faces").getAll();
          allReq.onsuccess = function() {
            var allFaces = allReq.result;
            if (allFaces.length === 0) {
              setStatus("No faces to compact.");
              return;
            }
            allFaces.sort(function(a, b) {
              return a.faceId - b.faceId;
            });
            var oldShards = /* @__PURE__ */ new Set();
            for (var i = 0; i < allFaces.length; i++) {
              oldShards.add(Math.floor(Math.log2(allFaces[i].faceId / 1e3 + 1)));
            }
            var idMap = {};
            for (var j = 0; j < allFaces.length; j++) {
              idMap[allFaces[j].faceId] = j + 1;
            }
            var newShards = /* @__PURE__ */ new Set();
            for (var k = 0; k < allFaces.length; k++) {
              newShards.add(Math.floor(Math.log2((k + 1) / 1e3 + 1)));
            }
            setStatus("Rewriting " + allFaces.length + " faces (" + oldShards.size + " \u2192 " + newShards.size + " shards)\u2026");
            var writeTx = db.transaction("faces", "readwrite");
            var store = writeTx.objectStore("faces");
            var clearReq = store.clear();
            clearReq.onsuccess = function() {
              for (var m = 0; m < allFaces.length; m++) {
                var f = allFaces[m];
                var newId = idMap[f.faceId];
                store.put({
                  faceId: newId,
                  cardId: f.cardId,
                  contents: f.contents,
            ...
            writeTx.oncomplete = function() {
              try {
                var journalKey = "umemo_journal_" + deckId;
                localStorage.removeItem(journalKey);
              } catch (e) {
              }
              var ds = window._driveSyncModule;
              if (ds && ds.clearRemoteDeck) {
                setStatus("Clearing remote sync state\u2026");
                ds.clearRemoteDeck(deckId).then(function() {
                  setStatus("\u2713 Compacted " + allFaces.length + " faces: " + oldShards.size + " \u2192 " + newShards.size + " shards. Remote cleared; sync to re-push.");
                }).catch(function(err) {
                  setStatus("\u2713 Compacted locally (" + oldShards.size + " \u2192 " + newShards.size + " shards). Remote clear failed: " + err.message + " \u2014 sync manually.");
                });
              } else {
                setStatus("\u2713 Compacted " + allFaces.length + " faces: " + oldShards.size + " \u2192 " + newShards.size + " shards. Next sync will do a full re-push.");
              }
            };
            writeTx.onerror = function(e) {
              setStatus("Error: " + e.target.error);
            };
          };
        };
        openReq.onerror = function(e) {
          setStatus("Error opening DB: " + e.target.error);
        };
      };
    })();
  }
    ...
    return function(totalCards) {
      return function(editable) {
        return function() {
          ...
          window._toggleDetail = function(on) {
            window._detailedView = on;
            window._goToCardPage(_currentPage);
          };
        };
      };
    };
  ...
  function _renderCardHeadHtml(deckId, cardId) {
    return `<div class="card-head"><button onclick="window._addFace('text',` + cardId + `)">Text</button><button onclick="window._addFace('markdown',` + cardId + `)">Markdown</button><button onclick="window._addFace('image',` + cardId + `)">Image</button><button onclick="window._addFace('audio',` + cardId + `)">Audio</button><button onclick="window._addFace('draw',` + cardId + ')">Draw</button></div>';
  }
  function _renderFaceHtml(f, editable, deckId) {
    ...
    } else if (faceType === "DrawFace") {
      if (editable) {
        content = '<div class="face-draw-edit"><input type="text" value="' + _esc(c.label || "") + '" placeholder="Draw prompt (optional)" onblur="window._saveFace(' + fid + `,this.value,'DrawFace')"><p class="face-text">\u270F Draw face</p></div>`;
      } else {
        var drawLabel = c.label ? "\u270F " + _esc(c.label) : "\u270F Draw";
        content = '<p class="face-text">' + drawLabel + "</p>";
      }
    }
    if (editable) {
    ...
    var detail = "";
    if (window._detailedView) {
      var pos = f.position != null ? f.position.toFixed(1) : "\u2014";
      var mom = f.momentum != null ? f.momentum.toFixed(2) : "\u2014";
      var linked = c.link_siblings !== false ? "linked" : "unlinked";
      detail = '<div class="face-detail">pos: ' + pos + " \xB7 mom: " + mom + " \xB7 " + linked + "</div>";
    }
    return '<div class="face" data-face-id="' + fid + '">' + content + detail + guts + "</div>";
  }
  function _esc(s) {
            ...
            return function() {
              ...
              Port.getFaceById(deckId)(faceId)(function(face) {
                return function() {
                  ...
                  var ds = window._driveSyncModule;
                  if (ds && ds.markContentDirty) ds.markContentDirty(deckId);
                };
              })();
            };
          };
          ...
          return function() {
            ...
            switch (faceType) {
              ...
              case "audio":
                ...
                break;
              case "draw":
                contents = { type: "DrawFace", label: "", priority: 1, momentum: 1, link_siblings: true };
                break;
              default:
                contents = { type: "TextFace", text: "New card", priority: 1, momentum: 1, link_siblings: true };
            }
    ...
    return function(deckId) {
      return function() {
        ...
        Port.randomUint32(function(cardId) {
          return function() {
            Port.getMaxPosition(deckId)(function(maxPos) {
              return function() {
                var PRIORITY_MAX = 2147483647;
          ...
          return function() {
            ...
            sources.forEach(function(url) {
              ...
              fetch(csvUrl).then(function(r) {
                ...
                _importCsvToDeck(Port, deckId, r.csvText, r.sourceId, function(count) {
                  total += count;
                  remaining--;
  ...
  function _importCsvToDeck(Port, deckId, csvText, sourceId, onDone) {
    ...
    if (window._showToast) window._showToast("Importing " + count + " cards\u2026", 0);
    var allFaces = [];
    var faceIdx = 0;
    var totalFaces = cards.reduce(function(s, c) {
      return s + c.faces.length;
    }, 0);
    var faceStep = PRIORITY_MAX / (totalFaces + 1);
    ...
    Port.saveFacesWithCallback(allFaces)(function() {
      return function() {
        Port.getDeckMeta(deckId)(function(meta) {
          return function() {
  ...
  function _parseCell(value) {
    ...
    if (value.startsWith("draw:")) {
      return { type: "DrawFace", label: value.substring(5).trim(), priority: 1, momentum: 1, link_siblings: true };
    }
          ...
          if (sheetUrl) {
            ...
            reader.onload = function(e) {
              ...
              crypto.subtle.digest("SHA-256", new TextEncoder().encode(file.name)).then(function(buf) {
                ...
                var sourceId = hex.substring(0, 16);
                _importCsvToDeck(Port, deckId, csvText, sourceId, function(count) {
                  alert("Imported " + count + " card(s).");
                  window.dispatchEvent(new Event("hashchange"));
                });
              });
  ...
  var saveSettingsFromJs = function() {
    var Port = window._portModule;
    if (!Port) return;
    var lr = parseFloat(document.getElementById("learning-rate").value) || 8;
  ...
  var currentVersion = "umemo-20260309211518";
  var checkForUpdates = function() {
    var status = document.getElementById("update-status");
    if (status) status.textContent = "Checking\u2026";
    fetch("./sw.js", { cache: "no-store" }).then(function(resp) {
      if (!resp.ok) throw new Error("HTTP " + resp.status);
      return resp.text();
    }).then(function(text) {
      var m = text.match(/CACHE_NAME\s*=\s*"([^"]+)"/);
      var serverVersion = m ? m[1] : null;
      if (!serverVersion) {
        if (status) status.textContent = "Could not determine server version.";
        return;
      }
      if (serverVersion === currentVersion) {
        if (status) status.textContent = "\u2713 You are on the latest version (" + currentVersion + ").";
        return;
      }
      if (status) status.innerHTML = "Update available: <code>" + serverVersion + "</code> (you have <code>" + currentVersion + "</code>). Updating\u2026";
      if ("serviceWorker" in navigator) {
        navigator.serviceWorker.getRegistration().then(function(reg) {
          ...
          reg.addEventListener("updatefound", function() {
            if (reg.installing) awaitInstalled(reg.installing);
          });
        });
      }
    }).catch(function(err) {
  ...
  var clearCacheAndReload = function() {
    var status = document.getElementById("update-status");
  ...
  function _enqueueSync(fn) {
    if (_syncQueued) return;
    _syncQueued = true;
    _syncQueue = _syncQueue.then(function() {
      _syncQueued = false;
  ...
  var startBackgroundSync = function(model) {
    return function() {
      ...
      _enqueueSync(function(done) {
        if (window._setSyncState) window._setSyncState("syncing");
        var btn = document.getElementById("sync-btn");
        if (btn) btn.textContent = "\u{1F504} Syncing\u2026";
        window._showToast("Syncing\u2026", 0);
        DS.fullSync(function() {
          return function() {
            if (window._setSyncState) window._setSyncState("done");
            var btn2 = document.getElementById("sync-btn");
            if (btn2) btn2.textContent = "\u2713 Synced";
            window._showToast("Synced \u2713", 2e3);
            ...
            setTimeout(function() {
              ...
              var btn3 = document.getElementById("sync-btn");
              if (btn3 && btn3.textContent === "\u2713 Synced") btn3.textContent = "\u2601 Sync";
            }, 3e3);
            done();
          };
        })(function(err) {
          return function() {
            ...
            var btn2 = document.getElementById("sync-btn");
            if (btn2) btn2.textContent = "\u26A0 Sync failed";
            window._showToast("Sync failed", 4e3);
            setTimeout(function() {
              if (window._setSyncState && window._syncState.status === "error")
                window._setSyncState("idle");
              var btn3 = document.getElementById("sync-btn");
              if (btn3 && btn3.textContent === "\u26A0 Sync failed") btn3.textContent = "\u2601 Sync";
            }, 5e3);
            done();
          };
        })();
  ...
  function _enqueuePushOnly(DS) {
    _enqueueSync(function(done) {
      if (window._setSyncState) window._setSyncState("syncing");
      window._showToast("Saving\u2026", 0);
      DS.pushSync(function() {
        return function() {
          if (window._setSyncState) window._setSyncState("done");
          window._showToast("Saved \u2713", 2e3);
          setTimeout(function() {
            if (window._setSyncState && window._syncState.status === "done")
              window._setSyncState("idle");
          }, 3e3);
        ...
        return function() {
          ...
          window._showToast("Save failed", 4e3);
          setTimeout(function() {
            if (window._setSyncState && window._syncState.status === "error")
              window._setSyncState("idle");
          }, 5e3);
  ...
  var DrawFace = /* @__PURE__ */ (function() {
    function DrawFace2(value0) {
      this.value0 = value0;
    }
    ;
    DrawFace2.create = function(value0) {
      return new DrawFace2(value0);
    };
    return DrawFace2;
  })();
  var decodeFaceType$prime = function(typeName) {
    return function(json) {
      return bind2(decodeJson2(json))(function(obj) {
        ...
        ;
        if (typeName === "DrawFace") {
          return bind2(map5(fromMaybe(""))(getFieldOptional$prime3(obj)("label")))(function(label) {
            return pure3(new DrawFace({
              label
            }));
          });
        }
  ...
  function _clearJournalSnapshot(deckId, snapshot) {
    var key = _journalKey(deckId);
    var current = [];
    try {
    ...
    var snapshotKeys = /* @__PURE__ */ new Set();
    for (var e of snapshot) {
      snapshotKeys.add(String(e.faceId) + ":" + String(e.timestamp || 0));
    }
    var remaining = current.filter(function(e2) {
      return !snapshotKeys.has(String(e2.faceId) + ":" + String(e2.timestamp || 0));
    });
  ...
  function _openDeckDB(deckId) {
    return new Promise(function(resolve, reject) {
      if (_dsDeckDBs[deckId]) {
        try {
          var name = _dsDeckDBs[deckId].name;
          if (_dsDeckDBs[deckId].objectStoreNames.contains("faces")) {
      ...
      var req = indexedDB.open("umemo-deck-" + deckId, 2);
  ...
  async function _pushDeck(folderId, deckId) {
    var journalSnapshot = _getJournal(deckId);
    ...
    if (!contentFile) {
      ...
      _clearJournalSnapshot(deckId, journalSnapshot);
      return;
    }
    ...
    if (journalSnapshot.length === 0) return;
    ...
    var merged = _dedupeJournal(remoteJournal.concat(journalSnapshot));
        ...
        _ensureToken().then(function() {
          ...
          try {
            ...
            for (var d of decks) {
              ...
              if (_isContentDirty(d.id)) {
                var content = await _exportDeckContent(d.id);
                await _uploadFile(folderId, "deck-" + d.id + "-content.json", content);
                _clearContentDirty(d.id);
              }
            ...
            for (var d2 of decks) {
              if (_isLocallyDeleted(d2.id)) continue;
              var existing = (manifest.decks || {})[d2.id];
              if (existing && _isDeckAlive(existing)) {
                localManifest.decks[d2.id] = existing;
              } else if (!existing) {
                localManifest.decks[d2.id] = { createdAt: Date.now() };
              } else if (existing && !_isDeckAlive(existing)) {
                localManifest.decks[d2.id] = { createdAt: Date.now() };
              }
            }
            var deletedIds = _getLocallyDeletedIds();
            for (var delId of deletedIds) {
              var remoteEntry = (manifest.decks || {})[delId];
              if (remoteEntry && _isDeckAlive(remoteEntry)) {
                localManifest.decks[delId] = Object.assign({}, remoteEntry, {
                  deletedAt: Date.now()
                });
                try {
                  await _deleteFileByName(folderId, "deck-" + delId + "-content.json");
                  await _deleteFileByName(folderId, "deck-" + delId + "-state.json");
                  await _deleteFileByName(folderId, "deck-" + delId + "-journal.json");
                  var shardFiles = await _listShardFiles(folderId, delId);
                  ...
                  console.error("Cleanup failed for deleted deck:", delId, e2);
                }
              }
              _unmarkLocallyDeleted(delId);
            }
  ...
  async function _pullDeck(folderId, deckId) {
    ...
    await new Promise(function(resolve) {
      ...
      if (content.meta) {
        ...
        if (content.faces) {
          var uniqueCards = new Set(content.faces.map(function(f) {
            return f.cardId;
          }));
          metaRecord.cardCount = uniqueCards.size;
        }
    ...
    if (content.faces) {
      var contentDirty = _isContentDirty(deckId);
      var PRIORITY_MAX = 2147483647;
      var totalFaceCount = content.faces.length;
      var step = PRIORITY_MAX / (totalFaceCount + 1);
      var existingFaces = await new Promise(function(resolve) {
        var tx = deckDB.transaction("faces", "readonly");
        var req = tx.objectStore("faces").getAll();
        req.onsuccess = function() {
          var map7 = {};
          for (var ef of req.result || []) {
            map7[ef.faceId] = ef;
          }
          resolve(map7);
        };
      ...
      await new Promise(function(resolve) {
        ...
        for (var fi = 0; fi < content.faces.length; fi++) {
          var f = content.faces[fi];
          var s = mergedState[f.faceId] || {};
          var existing = existingFaces[f.faceId];
          if (existing && existing._ts && existing._ts > (s.ts || 0)) {
            var useContents = contentDirty ? existing.contents || f.contents : f.contents;
            faceStore.put({
              faceId: f.faceId,
              cardId: f.cardId,
            ...
            var defaultPos = step * (fi + 1);
            var useContents2 = contentDirty && existing && existing.contents ? existing.contents : f.contents;
            faceStore.put({
              faceId: f.faceId,
              cardId: f.cardId,
              contents: useContents2,
              position: s.position !== void 0 ? s.position : defaultPos,
  ...
  function _getLocallyDeletedIds() {
    try {
      return Object.keys(JSON.parse(localStorage.getItem("umemo_deleted_decks") || "{}"));
    } catch (e) {
      return [];
    }
  }
  ...
  function _markContentDirty(deckId) {
    try {
      var set = JSON.parse(localStorage.getItem("umemo_content_dirty") || "{}");
      set[deckId] = Date.now();
      localStorage.setItem("umemo_content_dirty", JSON.stringify(set));
    } catch (e) {
    }
  }
  function _isContentDirty(deckId) {
    try {
      var set = JSON.parse(localStorage.getItem("umemo_content_dirty") || "{}");
      return !!set[deckId];
    } catch (e) {
  ...
  function _clearContentDirty(deckId) {
    try {
      var set = JSON.parse(localStorage.getItem("umemo_content_dirty") || "{}");
      delete set[deckId];
      localStorage.setItem("umemo_content_dirty", JSON.stringify(set));
  ...
  async function _clearRemoteDeck(deckId) {
    var token = _getToken();
    if (!token) throw new Error("Not connected to Drive");
    var folderId = await _getOrCreateAppFolder();
  ...
  window._driveSyncModule = {
    ...
    pullSync,
    pushDeckContent,
    clearRemoteDeck: _clearRemoteDeck,
    markContentDirty: _markContentDirty
  };
  window._setSyncState = _setSyncState;
  window._updatePlaySyncPopover = _updatePlaySyncPopover;
  window._markDeckLocallyDeleted = _markLocallyDeleted;
  ...
  function openDeckDB(deckId) {
    return new Promise((resolve, reject) => {
      if (_deckDBs[deckId]) {
        try {
          var name = _deckDBs[deckId].name;
          if (_deckDBs[deckId].objectStoreNames.contains("faces")) {
      ...
      const req = indexedDB.open("umemo-deck-" + deckId, 2);
      req.onupgradeneeded = (e) => {
        const db = e.target.result;
        if (!db.objectStoreNames.contains("faces")) {
      ...
      req.onblocked = () => {
        console.warn("IDB blocked for umemo-deck-" + deckId);
        if (_deckDBs[deckId]) {
          try {
  ...
  var getAllDeckFaces = function(deckId) {
    return function(callback) {
      return function() {
        openDeckDB(deckId).then((db) => {
          const tx = db.transaction("faces", "readonly");
          const req = tx.objectStore("faces").getAll();
          req.onsuccess = () => {
            const faces = req.result.map((f) => Object.assign({ deckId }, f));
            callback(faces)();
          };
        });
      };
  ...
  function _getAllByCardId(store, cardId, deckId, cb) {
    const idx = store.index("cardId");
    const range3 = IDBKeyRange.only(cardId);
    const req = idx.openCursor(range3);
    const results = [];
    req.onsuccess = () => {
      const cursor = req.result;
      if (cursor) {
        results.push(Object.assign({ deckId }, cursor.value));
        cursor.continue();
      } else {
        cb(results);
      }
      ...
      return function(callback) {
        return function() {
          openDeckDB(deckId).then((db) => {
            try {
              const tx = db.transaction("faces", "readonly");
              _getAllByCardId(tx.objectStore("faces"), cardId, deckId, function(results) {
                callback(results)();
              });
            } catch (e) {
              console.error("getCardFaces error:", e);
            }
          }).catch((e) => {
            console.error("getCardFaces openDB error:", e);
          });
        };
      };
    };
      ...
      return function() {
        openDeckDB(deckId).then((db) => {
          ...
          req.onsuccess = () => {
            ...
            _getAllByCardId(store, cardId, deckId, function(faces) {
              callback({
                empty: false,
                cardId,
                faces,
    ...
    return function(callback) {
      return function() {
        ...
        openDeckDB(deckId).then((db) => {
          ...
          tx.onerror = (e) => {
            console.error("saveFaces tx error:", e.target.error);
            callback()();
          };
          tx.onabort = (e) => {
            console.error("saveFaces tx abort:", e.target.error);
            callback()();
          };
        }).catch((e) => {
          console.error("saveFaces openDB error:", e);
        });
      };
    };
  };
              ...
              openDeckDB(deckId).then((db) => {
                ...
                for (let i = 0; i < facesJson.length; i++) {
                  const f = Object.assign({}, facesJson[i], {
                    cardId,
                    position: position != null ? position + i * 1e-3 : null,
          ...
          return function() {
            openDeckDB(deckId).then((db) => {
              ...
              req.onsuccess = () => {
                const cursor = req.result;
                if (!cursor) {
                  _fetchAllFacesForCards(db, pageCardIds, callback);
                  return;
                }
  ...
  function _fetchAllFacesForCards(db, cardIds, callback) {
    if (cardIds.length === 0) {
      callback([])();
      return;
    }
  ...
  var getActiveFaceCount = function(deckId) {
    return function(callback) {
      return function() {
        openDeckDB(deckId).then((db) => {
          const tx = db.transaction("faces", "readonly");
              ...
              if (rank <= 0) {
                ...
                req.onsuccess = () => {
                  ...
                  callback({
                    posAtRank: null,
                    posAfterRank: null,
                    firstPos: c ? c.value.position : null,
                    lastPos: null,
                    total
                  })();
                };
              } else if (rank >= total - 1) {
                const req = idx.openCursor(range3, "prev");
                req.onsuccess = () => {
                  const c = req.result;
                  callback({
                    posAtRank: null,
                    posAfterRank: null,
                    firstPos: null,
                    lastPos: c ? c.value.position : null,
                    total
                  })();
                };
              } else {
  ...
  var getMaxPosition = function(deckId) {
    return function(callback) {
      return function() {
        openDeckDB(deckId).then((db) => {
          const tx = db.transaction("faces", "readonly");
          ...
          req.onsuccess = () => {
            const cursor = req.result;
            if (cursor && cursor.value.position != null) {
              callback(cursor.value.position)();
            } else {
              callback(0)();
            }
          };
    ...
    return function(callback) {
      return function() {
        openDeckDB(deckId).then((db) => {
          const tx = db.transaction("faces", "readonly");
          const store = tx.objectStore("faces");
          const allReq = store.count();
          const posIdx = store.index("position");
          const range3 = IDBKeyRange.lowerBound(0, true);
          const posReq = posIdx.count(range3);
          var totalCount = null, posCount = null;
          allReq.onsuccess = () => {
            totalCount = allReq.result;
            if (totalCount !== null && posCount !== null && posCount < totalCount) {
              callback(true)();
              return;
            }
          };
          posReq.onsuccess = () => {
            posCount = posReq.result;
            if (totalCount !== null && posCount !== null && posCount < totalCount) {
              callback(true)();
              return;
            }
            const curReq = posIdx.openCursor(range3);
            var prevPos = null;
            curReq.onsuccess = () => {
  ...
  var rebalanceDeck = function(deckId) {
    return function(callback) {
      return function() {
        openDeckDB(deckId).then((db) => {
          const readTx = db.transaction("faces", "readonly");
          const readReq = readTx.objectStore("faces").getAll();
          readReq.onsuccess = () => {
            const allFaces = readReq.result || [];
            if (allFaces.length === 0) {
              callback();
              return;
            }
            var positioned = allFaces.filter((f) => f.position != null).sort((a, b) => a.position - b.position);
            var unpositioned = allFaces.filter((f) => f.position == null);
            var cardBestPos = {};
            for (var p of positioned) {
              if (cardBestPos[p.cardId] === void 0 || p.position < cardBestPos[p.cardId]) {
                cardBestPos[p.cardId] = p.position;
              }
            }
            for (var u of unpositioned) {
              u._sortPos = cardBestPos[u.cardId] !== void 0 ? cardBestPos[u.cardId] + u.faceId * 1e-4 : Infinity;
            }
            unpositioned.sort((a, b) => a._sortPos - b._sortPos || a.faceId - b.faceId);
            var merged = [];
            var ui = 0;
            for (var pi = 0; pi < positioned.length; pi++) {
              while (ui < unpositioned.length && unpositioned[ui]._sortPos <= positioned[pi].position) {
                merged.push(unpositioned[ui++]);
              }
              merged.push(positioned[pi]);
            }
            while (ui < unpositioned.length) merged.push(unpositioned[ui++]);
            const PRIORITY_MAX = 2147483647;
            const delta = PRIORITY_MAX / (merged.length + 1);
            const tx = db.transaction("faces", "readwrite");
            const store = tx.objectStore("faces");
            for (var i = 0; i < merged.length; i++) {
              var face = merged[i];
              delete face._sortPos;
  ...
  var showAnswer = function __do() {
    ...
    addClassById("play-card-buttons")("answer")();
    return freezeDrawCanvas();
  };
  ...
  var renderPaginationControls = function(currentPage) {
    return function(totalPages) {
      ...
      var nextDisabled = (function() {
        var $64 = currentPage >= (totalPages - 1 | 0);
        if ($64) {
          return " disabled";
        }
        ;
  ...
  var renderNewCardButton = function(v) {
    return function(v1) {
      ...
      ;
      throw new Error("Failed pattern match at Main (line 618, column 1 - line 618, column 51): " + [v.constructor.name, v1.constructor.name]);
    };
  };
    ...
    return function(meta) {
      return function(syncResult) {
        ...
        var syncMsg = (function() {
          ...
          ;
          throw new Error("Failed pattern match at Main (line 721, column 15 - line 723, column 20): " + [syncResult.constructor.name]);
        })();
      ...
      return function(meta) {
        return function(result) {
          ...
          var resultHtml = (function() {
            ...
            ;
            throw new Error("Failed pattern match at Main (line 769, column 20 - line 771, column 22): " + [result.constructor.name]);
          })();
          return function __do3() {
    ...
    return function(editable) {
      return function(fid) {
        ...
        if (face.faceType instanceof ImageFace) {
          ...
          ;
          var $82 = face.faceType.value0.src === "";
          if ($82) {
        ...
        if (face.faceType instanceof AudioFace) {
          if (editable) {
            return '<div class="face-audio-edit">' + ('<input type="url" value="' + (escapeHtml(face.faceType.value0.src) + ('" placeholder="Audio URL" onblur="window._saveFace(' + (show2(fid) + (`,this.value,'AudioFace_src')">` + ('<input type="text" value="' + (escapeHtml(face.faceType.value0.label) + ('" placeholder="Label" onblur="window._saveFace(' + (show2(fid) + (`,this.value,'AudioFace_label')">` + ((function() {
              var $85 = face.faceType.value0.src !== "";
              if ($85) {
          ...
          var labelHtml = (function() {
            var $86 = face.faceType.value0.label === "";
            if ($86) {
              return "";
            }
            ;
          ...
          var $87 = face.faceType.value0.src === "";
          if ($87) {
        ...
        ;
        if (face.faceType instanceof DrawFace) {
          if (editable) {
            return '<div class="face-draw-edit">' + ('<input type="text" value="' + (escapeHtml(face.faceType.value0.label) + ('" placeholder="Draw prompt (optional)" onblur="window._saveFace(' + (show2(fid) + `,this.value,'DrawFace')"><p class="face-text">\u270F Draw face</p></div>`))));
          }
          ;
          var labelHtml = (function() {
            var $90 = face.faceType.value0.label === "";
            if ($90) {
              return "\u270F Draw";
            }
            ;
            return "\u270F " + escapeHtml(face.faceType.value0.label);
          })();
          return '<p class="face-text">' + (labelHtml + "</p>");
        }
        ;
        throw new Error("Failed pattern match at Main (line 669, column 47 - line 707, column 58): " + [face.faceType.constructor.name]);
      };
    };
  };
  var renderFaceContent = function(face) {
    ...
    ;
    if (face.faceType instanceof ImageFace) {
      var $95 = face.faceType.value0.src === "";
      if ($95) {
        return '<p class="face-text">(no image)</p>';
      }
      ;
    ...
    if (face.faceType instanceof AudioFace) {
      var labelHtml = (function() {
        var $97 = face.faceType.value0.label === "";
        if ($97) {
          return "";
        }
        ;
      ...
      var $98 = face.faceType.value0.src === "";
      if ($98) {
        return '<p class="face-text">(no audio)</p>';
      }
      ;
    ...
    if (face.faceType instanceof DrawFace) {
      var labelHtml = (function() {
        var $100 = face.faceType.value0.label === "";
        if ($100) {
          return "";
        }
        ;
        return '<p class="face-text draw-label">' + (escapeHtml(face.faceType.value0.label) + "</p>");
      })();
      return labelHtml + '<canvas class="draw-canvas" id="draw-canvas"></canvas><div class="draw-tools"><button onclick="window._drawUndo()">Undo</button><button onclick="window._drawClear()">Clear</button></div>';
    }
    ;
    throw new Error("Failed pattern match at Main (line 495, column 26 - line 507, column 219): " + [face.faceType.constructor.name]);
  };
  ...
  <div class="card-head">
    ...
    <button onclick="window._addFace('draw',` + (show2(cardId) + ')">Draw</button>\n  </div>')))))))));
    };
  };
  ...
  var renderPlayPage = function(model) {
    return function(ps) {
      return function __do3() {
        ...
        var isDrawFront = (function() {
          if (ps.frontFace instanceof Just) {
            if (ps.frontFace.value0.face.faceType instanceof DrawFace) {
              return true;
            }
            ;
            return false;
          ...
          ;
          throw new Error("Failed pattern match at Main (line 398, column 21 - line 402, column 25): " + [ps.frontFace.constructor.name]);
        })();
        var promptFace = (function() {
          if (isDrawFront) {
            return head(filter(function(f) {
              if (f.face.faceType instanceof DrawFace) {
                return false;
              }
              ;
              return true;
        ...
        var frontHtml = (function() {
          ...
          ;
          if (ps.frontFace instanceof Just) {
            if (promptFace instanceof Just) {
              return '<div class="face play-face" id="answer-face">' + (renderFaceContent(promptFace.value0.face) + (renderFaceContent(ps.frontFace.value0.face) + "</div>"));
            }
            ;
            if (promptFace instanceof Nothing) {
              return renderFaceHtml(ps.frontFace.value0.face)(true);
            }
            ;
            throw new Error("Failed pattern match at Main (line 411, column 19 - line 413, column 48): " + [promptFace.constructor.name]);
          }
          ;
          throw new Error("Failed pattern match at Main (line 409, column 19 - line 413, column 48): " + [ps.frontFace.constructor.name]);
        })();
        var backsHtml = foldl2(function(acc) {
          return function(f) {
            var isPrompt = (function() {
              if (promptFace instanceof Just) {
                return f.faceId === promptFace.value0.faceId;
              }
              ;
              if (promptFace instanceof Nothing) {
                return false;
              ...
              ;
              throw new Error("Failed pattern match at Main (line 419, column 24 - line 421, column 31): " + [promptFace.constructor.name]);
            })();
            var isDrawType = (function() {
              if (f.face.faceType instanceof DrawFace) {
                return true;
              }
              ;
  ...
  var decodeSingleStoredFace = function(json) {
    ...
    if (v instanceof Right) {
      ...
      if (v1 instanceof Right) {
        ...
        ;
        throw new Error("Failed pattern match at Main (line 384, column 16 - line 386, column 24): " + [v2.constructor.name]);
      }
      ;
      ...
      ;
      throw new Error("Failed pattern match at Main (line 379, column 16 - line 387, column 22): " + [v1.constructor.name]);
    }
    ;
    if (v instanceof Left) {
      return Nothing.value;
    ...
    ;
    throw new Error("Failed pattern match at Main (line 378, column 31 - line 388, column 20): " + [v.constructor.name]);
  };
  var renderStoredFaceHtml = function(json) {
    return function(editable) {
      return function(deckId) {
        ...
        ;
        throw new Error("Failed pattern match at Main (line 661, column 45 - line 666, column 16): " + [v.constructor.name]);
      };
    };
  };
    ...
    return function(editable) {
      return function(deckId) {
        ...
        if (v instanceof Right) {
          ...
          ;
          throw new Error("Failed pattern match at Main (line 638, column 16 - line 647, column 17): " + [v1.constructor.name]);
        }
        ;
        if (v instanceof Left) {
          return "";
        ...
        ;
        throw new Error("Failed pattern match at Main (line 637, column 39 - line 648, column 15): " + [v.constructor.name]);
      };
    };
  };
        ...
        return function(totalCards) {
          return function(cardsJson) {
            return function(editable) {
              ...
              var bulkActionsHtml = (function() {
                if (editable) {
                  return `
    <details class="bulk-actions">
      <summary>Bulk Actions</summary>
      <div class="bulk-actions-body">
        <fieldset><legend>Select faces</legend>
          <label>Filter: <select id="bulk-filter">
            <option value="all">All faces</option>
            <option value="linked">Linked faces</option>
            <option value="unlinked">Unlinked faces</option>
            <option value="type">Face type\u2026</option>
            <option value="momentum-range">Momentum range\u2026</option>
            <option value="text-contains">Text contains\u2026</option>
          </select></label>
          <span id="bulk-filter-extra"></span>
          <p id="bulk-match-count" class="bulk-status"></p>
        </fieldset>
        <fieldset><legend>Action</legend>
          <label>Apply: <select id="bulk-action">
            <option value="link">Link momentum</option>
            <option value="unlink">Unlink momentum</option>
            <option value="set-momentum">Set momentum\u2026</option>
            <option value="set-priority">Set priority\u2026</option>
            <option value="reset-position">Reset positions (re-shuffle)</option>
          </select></label>
          <span id="bulk-action-extra"></span>
        </fieldset>
        <button type="button" class="btn-primary" onclick="window._bulkPreview()">Preview</button>
        <button type="button" class="btn-primary" id="bulk-apply-btn" onclick="window._bulkApply()" disabled>Apply to <span id="bulk-apply-count">0</span> faces</button>
        <p id="bulk-result" class="bulk-status"></p>
        <hr style="margin:1em 0;opacity:0.3">
        <fieldset><legend>Maintenance</legend>
          <button type="button" onclick="window._groupByCard()">Group faces by card</button>
          <p style="font-size:0.85em;opacity:0.7;margin:0.3em 0 0.5em">Sets each face's position to the minimum among its card's siblings, clustering them together.</p>
          <button type="button" onclick="window._rebalanceDeck()">Rebalance positions</button>
          <p style="font-size:0.85em;opacity:0.7;margin:0.3em 0 0.5em">Evenly redistributes face positions (fixes null positions from old data).</p>
          <button type="button" onclick="window._compactDeck()">Compact shard indices</button>
          <p style="font-size:0.85em;opacity:0.7;margin:0.3em 0 0.5em">Reassigns face IDs to be contiguous, reducing sync shard count. Requires a full re-sync.</p>
          <p id="compact-result" class="bulk-status"></p>
        </fieldset>
      </div>
    </details>`;
                }
              ...
              return function __do3() {
                setInnerHTML("app")('\n    <main class="container">\n      <div class="page-nav">' + (navLinks + ("</div>\n      <h1>" + (editPrefix + (escapeHtml(title) + ('</h1>\n      <p class="deck-card-count">' + (show2(totalCards) + (' cards\n        <label style="margin-left:1em;font-size:0.85em"><input type="checkbox" id="detail-toggle" onchange="window._toggleDetail&&window._toggleDetail(this.checked)"> Detailed</label>\n      </p>\n      ' + (metaFormHtml + ("\n      " + (bulkActionsHtml + ("\n      " + (paginationHtml + ('\n      <div id="card-list-container">' + (cardsHtml + ("</div>\n      " + (paginationHtml + ("\n      " + (renderNewCardButton(editable)(deckId) + "\n    </main>\n  ")))))))))))))))))))();
                if (editable) {
  ...
  var renderSettingsPage = function(_model) {
    return loadSettings(function(settingsJson) {
      ...
      return function __do3() {
        setInnerHTML("app")('<main class="container">' + ('<div class="page-nav"><a href="#/" class="nav-link">\u2190 Home</a></div>' + ("<h1>\u2699 Settings</h1>" + ('<form onsubmit="event.preventDefault(); window._saveSettings()">' + ("<fieldset><legend>Scheduler</legend>" + ('<label for="learning-rate">Study intensity</label>' + ('<input id="learning-rate" name="learning_rate" type="number" step="0.1" min="2.0" max="20.0" value="' + (show1(settings.learningRate) + ('">' + ("<small>Total encounters per card before it reaches the back of the deck. Default: 8</small>" + ("</fieldset>" + ("<fieldset><legend>Face defaults</legend>" + ('<label for="default-priority">Default priority</label>' + ('<input id="default-priority" name="default_face_priority" type="number" step="0.1" min="0.1" value="' + (show1(settings.defaultFacePriority) + ('">' + ("<small>Priority weight for new faces.</small><br>" + ('<label><input id="default-link-siblings" name="default_link_siblings" type="checkbox" ' + (checkedAttr + ("> Link sibling faces by default</label>" + ("<small>When checked, voting on one face affects momentum of all sibling faces.</small>" + ("</fieldset>" + ('<button type="submit">Save</button>' + ("</form>" + ('<fieldset style="margin-top:1.5em"><legend>App</legend>' + ('<p id="version-info" style="margin:0 0 0.5em">Current version: <code>' + (currentVersion + '</code></p><button type="button" onclick="window._checkForUpdates()">Check for updates</button> <button type="button" onclick="window._clearCacheAndReload()">Force refresh</button><p id="update-status" style="margin-top:0.5em;font-size:0.85em"></p></fieldset></main>')))))))))))))))))))))))))))();
        setGlobal("_saveSettings")(saveSettingsFromJs)();
        setGlobal("_checkForUpdates")(checkForUpdates)();
        return setGlobal("_clearCacheAndReload")(clearCacheAndReload)();
      };
    });
  };
  var decodeFaceArray = function(arr) {
    return concatMap(function(j) {
      ...
      ;
      throw new Error("Failed pattern match at Main (line 373, column 40 - line 375, column 16): " + [v.constructor.name]);
    })(arr);
  };
    ...
    return function(deckTitle) {
      return function(json) {
        ...
        if (v instanceof Right) {
          ...
          ;
          throw new Error("Failed pattern match at Main (line 350, column 12 - line 367, column 53): " + [v2.constructor.name]);
        }
        ;
        if (v instanceof Left) {
          return {
        ...
        ;
        throw new Error("Failed pattern match at Main (line 345, column 3 - line 370, column 49): " + [v.constructor.name]);
      };
    };
  };
    ...
    return function(decks) {
      return function __do3() {
        ...
        var pendingLabel = (function() {
          var $182 = details.pendingEntries > 0;
          if ($182) {
        ...
        var syncButton = (function() {
          if (connected) {
            return '<span class="sync-btn-wrap">\n                <a href="javascript:void(0)" onclick="window._driveSync()" class="toolbar-link" id="sync-btn">' + (statusIcon + (" Sync</a>" + (syncPopover + '</span><a href="javascript:void(0)" onclick="window._driveDisconnect()" class="toolbar-link">\u{1F50C} Disconnect</a>')));
          }
          ;
      ...
      <details class="about-section">
        ...
        <h4>How to study</h4>
        <p>Each card in a deck has one or more <em>faces</em> \u2014 pieces of
        information you want to associate together. When you play a deck,
        one face is shown as a prompt. Try to recall the other faces from
        memory, then tap <strong>Show</strong> to reveal them all. Rate your
        recall: <strong>Bad</strong> if you couldn't remember,
        <strong>Neutral</strong> if you partially remembered, or
        <strong>Good</strong> if you knew it well. The app uses your rating to
        schedule when you'll see that face again \u2014 faces you struggle with
        appear sooner, while ones you know well are pushed further back.</p>
        <h4>Face types</h4>
        <p>Faces can hold different kinds of content. <strong>Text</strong>
        and <strong>Markdown</strong> faces display written content.
        <strong>Image</strong> faces show a picture from a URL.
        <strong>Audio</strong> faces embed a playable audio clip.
        <strong>Draw</strong> faces present a blank canvas where you
        practice writing or sketching from memory before revealing the
        answer.</p>
        <ul>
